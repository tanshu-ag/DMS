      ...
      case "customer_name":
        return (
          <div className="flex items-center gap-2">
            ...
            {appt.is_rescheduled && (
              <div className="w-5 h-5 bg-amber-100 text-amber-800 rounded-sm flex items-center justify-center text-xs font-bold shrink-0" title={appt.reschedule_history?.map(h => `${h.from_date} → ${h.to_date}`).join(", ") || "Rescheduled"}>R</div>
            )}
          </div>
        );
      case "phone":
