...
const MonthView = () => {
  ...
  const fetchData = async () => {
    setLoading(true);
    try {
      const params = new URLSearchParams({ view: "month", month, year });
  ...
  const handleExport = async () => {
    ...
    try {
      const response = await axios.get(
        `${API}/export/csv?view=month&month=${month}&year=${year}`,
  ...
  return (
    <div className="space-y-6" data-testid="month-view-page">
      {/* Header */}
      <div className="flex flex-col md:flex-row md:items-center md:justify-between gap-4">
        <div>
          ...
          <div className="flex items-center gap-4 mt-2">
            <Button variant="ghost" size="icon" className="rounded-sm" onClick={prevMonth}>
              <ChevronLeft className="w-5 h-5" strokeWidth={1.5} />
            </Button>
            <span className="font-mono text-lg font-bold">
              {monthNames[month - 1]} {year}
            </span>
            <Button variant="ghost" size="icon" className="rounded-sm" onClick={nextMonth}>
        ...
        <CardContent className="p-4">
          ...
          <div className="grid grid-cols-2 md:grid-cols-4 lg:grid-cols-6 gap-3">
            ...
            <Select
              ...
              onValueChange={(v) => setFilters({ ...filters, docket: v === "all" ? "" : v })}
            >
              <SelectTrigger className="rounded-sm h-9 text-sm">
                <SelectValue placeholder="Docket Ready" />
              </SelectTrigger>
              <SelectContent>
                <SelectItem value="all">All</SelectItem>
                <SelectItem value="true">Docket Ready</SelectItem>
              </SelectContent>
            </Select>

            <Select
              value={filters.recovered}
              onValueChange={(v) => setFilters({ ...filters, recovered: v === "all" ? "" : v })}
            >
              <SelectTrigger className="rounded-sm h-9 text-sm">
                <SelectValue placeholder="Recovered" />
              </SelectTrigger>
              <SelectContent>
                <SelectItem value="all">All</SelectItem>
                <SelectItem value="true">Recovered Only</SelectItem>
              </SelectContent>
            </Select>
          </div>
        </CardContent>
