...
export default function Reception() {
  ...
  const selectVehicle = async (index) => {
    ...
    // Track if Reg No was originally missing
    const regNoMissing = !v.vehicle_reg_no;
    setSelectedRegNoMissing(regNoMissing);
    setSelectedRegNo(v.vehicle_reg_no || "");
  ...
  const resetWizard = () => {
    ...
    setSelectedVehicleIndex(-1); setSelectedVin(""); setSelectedVinMissing(false); 
    setSelectedRegNo(""); setSelectedRegNoMissing(false); setCreateNew(false);
  ...
  const canGoNext1 = selectedVehicle ? (getSelectedRegNo() && getSelectedVin()) : (createNew && newVehicle.vehicle_reg_no && newVehicle.vin && !dupError.reg && !dupError.vin);
                ...
                <div className="space-y-2">
                  <div className="max-h-48 overflow-y-auto space-y-2">
                    {searchResults.map((r, i) => (
                      <div key={i} className={`p-3 border rounded-sm cursor-pointer transition-colors ${selectedVehicleIndex === i ? "border-black bg-gray-50" : "border-gray-200 hover:border-gray-400"}`}
                        onClick={() => {
